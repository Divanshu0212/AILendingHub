...
| LH-103 | Credit Policy + Finance Controller | Appendix A *Default / Bad* completeness | CBS reason codes that mark (a) distress restructuring and (b) write-off. The definition is `[SPEC]`; the code set that identifies it in the source system is a bank mapping. Master Appendix A v1.1 now marks both arms `[POLICY]`, and `pending_definitions()` reports *Default / Bad* as unresolved. | open |
