...
11. [Cross-Cutting Concerns — MLOps, Model Risk, Fairness, Privacy, Security](#11-cross-cutting-concerns)
12. [Non-Functional Requirements](#12-non-functional-requirements)
13. [Phased Implementation Roadmap](#13-phased-implementation-roadmap)
14. [Consolidated References](#14-consolidated-references)
...
- **Discrimination:** AUC/Gini, KS statistic, Gini by segment and vintage. Computed on the **raw model score**, not the calibrated PD. A monotone step calibrator (isotonic) quantises the score — it cannot reorder, but it collapses distinct values into ties, and ties cost Gini. Measured on the PD, discrimination falls by an amount that depends on how many rows the calibrator saw, which reports the calibration sample size as though it were a property of the model.
- **Calibration:** Brier score, Hosmer–Lemeshow by decile, ECE. Computed on the **calibrated PD**, which is the artifact that reaches pricing, the bands and the customer. Population stability is also measured here, for the same reason: PSI on the deployed score, not on an intermediate one.
- **Ties:** every rank-based metric must handle them explicitly. Ordering tied scores by outcome — which `sort` on a `(score, label)` pair does silently — walks every negative of a tie group past every positive and reports the separation a strict ordering would have given. It flatters exactly the models most likely to tie: shallow trees, and anything predicting a constant.
...
### 11.2 Model risk management & regulatory alignment
...
### 11.3 Fairness & customer protection
...
| Accessibility & language | Assistant + customer notices in local languages; WCAG-conformant dashboards |
...
| **P3 — Portfolio brain** | 3 mo | Module 5 (behavioral PD, survival, LGD/EAD, staging); Module 7 dashboards | PD backtest within tolerance; dashboards live at ≤ 5 min freshness |
| **P4 — EWS + recommendations** | 3 mo | Module 8 signals + BOCPD + action library; Module 4 stages 1–3 | ≥ 55% capture rate at 60-day lead in backtest; recommendation A/B live |
| **P5 — GenAI assistant** | 2–3 mo (parallel from P3) | Module 6 RAG + tools + guardrails, officer-facing first, then customer-facing | Faithfulness ≥ 97% on golden set; red-team pass |
