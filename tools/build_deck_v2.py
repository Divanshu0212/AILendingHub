...
def model_detail(prs):
    """Every model trained, per family. The evidence behind slide 8's four rows."""
    s = blank(prs)
    slide_num(s, "10")
    eyebrow(s, "Every model, not just the winners")
    heading(s, "Nineteen models across four families")

    tf = box(s, M, Inches(1.85), Inches(11.5), Inches(0.4))
    para(tf, "Each family fits several models and keeps the best. Showing all of "
             "them is the difference between a result and a claim: a single "
             "number could be luck, and a spread shows whether the family "
             "agrees with itself.",
         size=12, color=INK2, first=True, space_after=0, spacing=1.25)

    groups = [
        ("CREDIT SCORING · Home Credit, 307,511 applications", [
            ("xgboost_hist", "0.7794"), ("xgboost_deep", "0.7794"),
            ("lightgbm_deep", "0.7791"), ("lightgbm_wide", "0.7790"),
            ("lightgbm_shallow", "0.7783"), ("random_forest", "0.7575"),
            ("extra_trees", "0.6936"), ("→ logistic stack", "0.7797"),
        ]),
        ("FRAUD · IEEE-CIS, 590,540 transactions", [
            ("lightgbm", "0.9555"), ("xgboost", "0.9542"),
            ("catboost", "0.9508"), ("→ logistic stack", "0.9564"),
        ]),
        ("DEFAULT PREDICTION · Fannie Mae, 230,543 accounts", [
            ("xgboost_pd", "0.8818"), ("lightgbm_pd", "0.8813"),
            ("lightgbm_pd_deep", "0.8811"), ("gradient_boosting", "0.8789"),
            ("random_forest_pd", "0.8730"),
        ]),
        ("CROP · AgriFieldNet, 147,409 pixels  (macro-F1)", [
            ("extra_trees", "0.5249"), ("random_forest", "0.5163"),
            ("lightgbm", "0.5115"), ("xgboost", "0.4966"),
            ("→ equal-weight blend", "0.5225"),
        ]),
    ]
    ...
    for i, (title, rows) in enumerate(groups):
        col, row = i % 2, i // 2
        x = M + Inches(6.1) * col
        y = Inches(2.5) + Inches(2.35) * row
        tf = box(s, x, y, Inches(5.6), Inches(0.26))
        para(tf, title, size=9, color=BRAND_500, bold=True, font=MONO,
             first=True, space_after=0)
        for j, (name, score) in enumerate(rows):
            yy = y + Inches(0.38) + Inches(0.215) * j
            winner = name.startswith("→")
            tf = box(s, x + Inches(0.1), yy, Inches(3.6), Inches(0.2))
            para(tf, name, size=10, color=COMPUTED if winner else INK2,
                 bold=winner, font=MONO, first=True, space_after=0)
            tf = box(s, x + Inches(3.8), yy, Inches(1.2), Inches(0.2))
            para(tf, score, size=10, color=COMPUTED if winner else INK,
                 bold=winner, font=MONO, first=True, space_after=0)


def fraud_detail(prs):
    """The fraud result, and the operating decision behind it."""
    s = blank(prs)
    slide_num(s, "11")
    eyebrow(s, "Fraud detection · the strongest result")
    heading(s, "0.9564 AUC on 590,540 real card transactions")

    tf = box(s, M, Inches(1.9), Inches(6.1), Inches(1.5))
    para(tf, "Three model families on identical folds. The folds are by calendar "
             "month, so a model never sees a transaction from the month it is "
             "scored on — a random split would let a card's January purchases "
             "predict its own January purchases, which is the leak this "
             "competition is known for.",
         size=12, color=INK2, first=True, space_after=10, spacing=1.28)
    para(tf, "Per-fold AUC runs 0.929 to 0.971. That spread is the honest "
             "uncertainty on the headline number.",
         size=12, color=INK2, space_after=0, spacing=1.28)

    note(s, M, Inches(3.7), Inches(6.1), Inches(1.5),
         "The single biggest feature is a customer id the dataset does not "
         "ship. Subtracting a days-since-account-opened counter from the clock "
         "leaves a constant per card — which turns 590,540 independent-looking "
         "rows into customers with histories. Exactly the entity-resolution "
         "step Module 3 performs on shared phone, device and address.")

    x = Inches(7.4)
    tf = box(s, x, Inches(1.9), Inches(5.2), Inches(0.28))
    para(tf, "WHAT A REVIEW DESK WOULD SEE", size=9.5, color=INK3, font=MONO,
         first=True, space_after=0)

    heads = ["REVIEW", "CAUGHT", "PRECISION", "FALSE ALERTS"]
    widths = [Inches(1.0), Inches(1.2), Inches(1.4), Inches(1.6)]
    xx = x
    for h, w in zip(heads, widths):
        tf = box(s, xx, Inches(2.35), w, Inches(0.22))
        para(tf, h, size=8.5, color=INK3, font=MONO, first=True, space_after=0)
        xx += w
    rect(s, x, Inches(2.62), Inches(5.2), Emu(12700), fill=RGBColor(0xB9, 0xC8, 0xD8))

    budget = [("0.5%", "14.1%", "98.4%", "46"),
              ("1%", "27.6%", "96.4%", "211"),
              ("2%", "51.3%", "89.8%", "1,204"),
              ("5%", "74.1%", "51.9%", "14,214")]
    for r, cells in enumerate(budget):
        yy = Inches(2.78) + Inches(0.42) * r
        xx = x
        highlight = r == 1
        for c, (cell, w) in enumerate(zip(cells, widths)):
            tf = box(s, xx, yy, w, Inches(0.26))
            para(tf, cell, size=11.5,
                 color=COMPUTED if (highlight and c == 1) else
                       (BLOCKED if c == 3 and r == 3 else INK),
                 bold=highlight, font=MONO, first=True, space_after=0)
            xx += w
        rect(s, x, yy + Inches(0.3), Inches(5.2), Emu(9525), fill=RULE)

    note(s, x, Inches(4.7), Inches(5.2), Inches(1.55),
         "This table is the product, not the AUC. A desk cannot act on a "
         "ranking — it acts on a budget. Review 1% of transactions and you "
         "catch a quarter of the fraud with 211 wrong stops. Review 5% and you "
         "catch three quarters, and stop 14,214 good customers. Where that line "
         "sits is a business decision, and it is the one this platform refuses "
         "to make on a bank's behalf.", kind="warn")


def data_slide(prs):
    """What was trained on, and what each dataset can and cannot support."""
    s = blank(prs)
    slide_num(s, "12")
    eyebrow(s, "The evidence base")
    heading(s, "Four real datasets, and the limit of each")

    rows = [
        ("Home Credit", "307,511 loan applications with repayment outcomes",
         "Consumer credit, not agricultural. The label is the publisher's "
         "definition of payment difficulty, not this bank's default."),
        ("Fannie Mae", "16.8M monthly rows, 19 years, 230,543 accounts scored",
         "US mortgages. Real censoring, real seasoning, real crisis — but a "
         "different product and a different economy."),
        ("IEEE-CIS", "590,540 card transactions, 3.5% fraud, 6 months",
         "E-commerce card fraud, not loan fraud. The graph structure is the "
         "transferable part; the fraud patterns are not."),
        ("AgriFieldNet", "1,173 Sentinel-2 tiles, 147,409 labelled pixels, India",
         "Real Indian farmland with walked ground truth. Single-date imagery, "
         "which caps what any model can separate."),
    ]
    y = Inches(2.15)
    for name, what, limit in rows:
        rect(s, M, y + Inches(0.04), Inches(0.03), Inches(0.9), fill=BRAND_500)
        tf = box(s, M + Inches(0.22), y, Inches(2.3), Inches(0.26))
        para(tf, name, size=13, color=INK, bold=True, first=True, space_after=0)
        tf = box(s, M + Inches(0.22), y + Inches(0.28), Inches(4.6), Inches(0.5))
        para(tf, what, size=10.5, color=BRAND_700, font=MONO, first=True,
             space_after=0, spacing=1.15)
        tf = box(s, Inches(5.9), y, Inches(6.7), Inches(0.9))
        para(tf, limit, size=11, color=INK2, first=True, space_after=0,
             spacing=1.25)
        y += Inches(1.12)

    note(s, M, Inches(6.75), Inches(11.9), Inches(0.62),
         "None of these is this bank's book, and the platform says so on every "
         "screen that renders a number from them.", kind="warn")
...
def stack_slide(prs):
    ...
    heading(s, "The engineering underneath")

    cols = [
        ("BACKEND", BRAND_700, [
            ("21 packages", "~40,700 lines"),
            ("Standard-library Python", "no framework, runs anywhere"),
            ("2,331 tests", "green on every commit"),
            ("Every algorithm a port", "names the library it replaces"),
        ]),
        ("INTERFACES", BRAND_500, [
            ("Next.js + TypeScript", "46 files, ~6,700 lines"),
            ("Nine screens", "officer, collections, risk, customer"),
            ("One API gateway", "never a model service directly"),
            ("Build gate on arithmetic", "the UI cannot compute a rupee"),
        ]),
        ("MACHINE LEARNING", COMPUTED, [
            ("19 models, 4 families", "LightGBM, XGBoost, CatBoost, forests"),
            ("Out-of-fold stacking", "blends fitted on unseen rows"),
            ("Time-based splits", "never random on a panel"),
            ("A live training monitor", "watch every fit as it runs"),
        ]),
    ]
    for i, (title, colour, items) in enumerate(cols):
        x = M + Inches(4.06) * i
        rect(s, x, Inches(1.95), Inches(3.7), Inches(0.045), fill=colour)
        tf = box(s, x, Inches(2.15), Inches(3.7), Inches(0.28))
        para(tf, title, size=10, color=colour, bold=True, font=MONO,
             first=True, space_after=0)
        for j, (bold, sub) in enumerate(items):
            yy = Inches(2.6) + Inches(0.86) * j
            tf = box(s, x, yy, Inches(3.6), Inches(0.26))
            para(tf, bold, size=12, color=INK, bold=True, first=True,
                 space_after=0)
            tf = box(s, x, yy + Inches(0.27), Inches(3.6), Inches(0.45))
            para(tf, sub, size=10, color=INK2, first=True, space_after=0,
                 spacing=1.15)

    note(s, M, Inches(6.25), Inches(11.9), Inches(0.72),
         "The backend uses no machine-learning library at all: every model is "
         "written from the paper so a reviewer can read it. The training "
         "harnesses that use LightGBM and XGBoost live outside that boundary "
         "and import nothing from it — which is exactly the swap the "
         "architecture was designed for.")


def architecture(prs):
    """How the pieces fit, as a diagram rather than a list."""
    s = blank(prs)
    slide_num(s, "07")
    eyebrow(s, "Architecture")
    heading(s, "One gateway, eight engines, four surfaces")

    # -- surfaces (top) --
    tf = box(s, M, Inches(1.85), Inches(11.5), Inches(0.24))
    para(tf, "INTERFACES", size=9, color=INK3, font=MONO, first=True, space_after=0)
    surfaces = ["Customer app", "Officer workbench", "Risk dashboards", "Collections"]
    for i, name in enumerate(surfaces):
        x = M + Inches(2.98) * i
        rect(s, x, Inches(2.15), Inches(2.75), Inches(0.5), fill=BRAND_50, line=BRAND_200)
        tf = box(s, x + Inches(0.15), Inches(2.29), Inches(2.5), Inches(0.24))
        para(tf, name, size=11, color=BRAND_900, bold=True, first=True,
             space_after=0, align=PP_ALIGN.CENTER)

    # -- gateway --
    rect(s, M, Inches(2.95), Inches(11.9), Inches(0.62), fill=BRAND_700)
    tf = box(s, M + Inches(0.25), Inches(3.06), Inches(11.4), Inches(0.24))
    para(tf, "DECISION ORCHESTRATOR  ·  every response carries its model id, "
             "version and decision-log reference",
         size=11.5, color=WHITE, bold=True, first=True, space_after=0,
         align=PP_ALIGN.CENTER)
    tf = box(s, M + Inches(0.25), Inches(3.3), Inches(11.4), Inches(0.22))
    para(tf, "a screen cannot be handed a number it cannot trace",
         size=9.5, color=BRAND_200, font=MONO, first=True, space_after=0,
         align=PP_ALIGN.CENTER)

    # -- engines --
    tf = box(s, M, Inches(3.78), Inches(11.5), Inches(0.24))
    para(tf, "ENGINES", size=9, color=INK3, font=MONO, first=True, space_after=0)
    engines = [
        ("Agri", "M1", "drought, crop, plots"),
        ("Scoring", "M2", "scorecard + GBM"),
        ("Fraud", "M3", "graph, velocity, docs"),
        ("Offers", "M4", "feasible set, pricing"),
        ("Default", "M5", "PD / LGD / EAD"),
        ("Assistant", "M6", "retrieval, citations"),
        ("Dashboards", "M7", "portfolio metrics"),
        ("Early warning", "M8", "change-point, alerts"),
    ]
    for i, (name, mod, sub) in enumerate(engines):
        col, row = i % 4, i // 4
        x = M + Inches(2.98) * col
        y = Inches(4.08) + Inches(0.88) * row
        rect(s, x, y, Inches(2.75), Inches(0.72), fill=None, line=RULE)
        rect(s, x, y, Inches(2.75), Inches(0.035), fill=COMPUTED)
        tf = box(s, x + Inches(0.15), y + Inches(0.14), Inches(2.4), Inches(0.24))
        para(tf, name, size=11.5, color=INK, bold=True, first=True, space_after=0)
        tf = box(s, x + Inches(2.2), y + Inches(0.14), Inches(0.5), Inches(0.2))
        para(tf, mod, size=9, color=BRAND_500, font=MONO, first=True, space_after=0)
        tf = box(s, x + Inches(0.15), y + Inches(0.4), Inches(2.4), Inches(0.24))
        para(tf, sub, size=9.5, color=INK3, first=True, space_after=0)

    # -- platform --
    rect(s, M, Inches(5.92), Inches(11.9), Inches(0.5), fill=BRAND_50, line=BRAND_200)
    tf = box(s, M + Inches(0.25), Inches(6.05), Inches(11.4), Inches(0.24))
    para(tf, "PLATFORM  ·  frozen definitions · lakehouse · identity spine · "
             "feature store · model registry · decision log · consent",
         size=10.5, color=BRAND_900, first=True, space_after=0,
         align=PP_ALIGN.CENTER)

    note(s, M, Inches(6.62), Inches(11.9), Inches(0.62),
         "Every definition — what counts as a default, what counts as a cure — "
         "is imported from one place. Changing one needs committee approval and "
         "an impact analysis on every model that imports it.")


def roadmap(prs):
    """What ships when, and what each stage needs."""
    s = blank(prs)
    slide_num(s, "19")
    eyebrow(s, "What happens next")
    heading(s, "From here to a customer")

    stages = [
        ("NOW", "Built and measured", COMPUTED, [
            "Nineteen models on real data",
            "Nine screens over one gateway",
            "2,331 tests, every commit",
            "98 blockers named with owners",
        ]),
        ("NEXT", "Needs a decision, not code", BLOCKED, [
            "Ratify cutoffs and risk bands",
            "Set the alert budget",
            "Supply the crop calendar",
            "Approve reason-code wording",
        ]),
        ("THEN", "Needs the bank's own data", BRAND_500, [
            "Retrain on this book",
            "Shadow-run beside the current process",
            "Independent model validation",
            "Fairness audit before any live decision",
        ]),
        ("LIVE", "Steady state", BRAND_700, [
            "Champion / challenger, forever",
            "Weekly drift and calibration checks",
            "Randomised holdouts for uplift",
            "Annual revalidation",
        ]),
    ]
    for i, (tag, title, colour, items) in enumerate(stages):
        x = M + Inches(3.03) * i
        rect(s, x, Inches(2.0), Inches(2.75), Inches(0.05), fill=colour)
        tf = box(s, x, Inches(2.22), Inches(2.6), Inches(0.24))
        para(tf, tag, size=10, color=colour, bold=True, font=MONO, first=True,
             space_after=0)
        tf = box(s, x, Inches(2.5), Inches(2.6), Inches(0.3))
        para(tf, title, size=13, color=INK, bold=True, font=DISPLAY, first=True,
             space_after=0)
        for j, item in enumerate(items):
            tf = box(s, x, Inches(2.95) + Inches(0.62) * j, Inches(2.65), Inches(0.5))
            para(tf, "· " + item, size=10.5, color=INK2, first=True,
                 space_after=0, spacing=1.2)

    note(s, M, Inches(5.75), Inches(11.9), Inches(0.85),
         "The second column is the honest bottleneck. None of it is engineering "
         "work — it is four committees writing down numbers they already have "
         "opinions about. Every one is registered with a named owner, which is "
         "why the platform can say precisely what it is waiting for.",
         kind="warn")

    note(s, M, Inches(6.75), Inches(11.9), Inches(0.62),
         "Nothing in the first column has to be unpicked to reach the fourth. "
         "The interfaces, the gateway and the definitions do not change when a "
         "cutoff is ratified — the value simply arrives.", kind="good")
...
def main() -> None:
    ...
    out = "/home/divanshu/Desktop/AILendingHub/Lending_Hub_EPIC_8_v2.pptx"
    prs.save(out)
    print(f"wrote {out}")
