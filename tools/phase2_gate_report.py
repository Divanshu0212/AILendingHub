...
def build() -> list[str]:
    ...
    add("## Where the data would come from")
    add("")
    add("[docs/phase2/DATA_SOURCING.md](../docs/phase2/DATA_SOURCING.md) sets out,")
    add("for each blocker, what public data exists and what a bank collects instead.")
    add("Its findings, each verified by downloading or requesting the resource:")
    add("")
    add("* **Model A's boundary blocker is a sourcing gap, not a structural one.**")
    add("  ~10,000 hand-delineated Indian field polygons are published under")
    add("  CC-BY-4.0 as a single 7.8 MB file — a hundred times what the IoU gate")
    add("  needs. They are photo-interpreted rather than walked, so they are a")
    add("  benchmark and not a registry source (LH-413).")
    add("* **Model B's is structural, and now measured.** CropHarvest — the dataset")
    add("  the phase file names — carries 95,186 labels of which **34 are")
    add("  crop-typed inside India**, across seven crops, against a criterion")
    add("  asking for five majority crops per zone.")
    add("* **Model C's district yields exist** (ICRISAT DLD, 1966-2015/16, 20")
    add("  crops), behind a form-driven portal rather than a URL.")
    add("* **LH-406 is confirmed structural.** RBI publishes agri NPA aggregates")
    add("  only; no loan-level agri credit outcomes are public anywhere. Inside a")
    add("  bank this is a data-*access* problem under LH-120, not a collection one.")
    add("")
    add("None of this moves a criterion, because every one depends on LH-406 or")
    add("LH-102. It moves the *reason* three of them are blocked, which is the")
    add("distinction this pack exists to keep straight.")
    add("")
